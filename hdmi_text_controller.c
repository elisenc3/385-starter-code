...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	if(color % 2 == 0){
		//hdmi_ctrl->palette[color/2] &= 0xFFFF0000;
		hdmi_ctrl->palette[color/2] &= 0xFFFF0000;
		//hdmi_ctrl->palette[color/2] |= red << 8;
		hdmi_ctrl->palette[color/2] |= red << 8;
		//hdmi_ctrl->palette[color/2] |=  green << 4 ;
		hdmi_ctrl->palette[color/2] |=  green << 4 ;
		//hdmi_ctrl->palette[color/2] |= blue;
		hdmi_ctrl->palette[color/2] |= blue;
	} else {
		//hdmi_ctrl->palette[color/2] &= 0x0000FFFF;
		hdmi_ctrl->palette[color/2] &= 0x0000FFFF;
		//hdmi_ctrl->palette[color/2] |= red << 24 ;
		hdmi_ctrl->palette[color/2] |= red << 24 ;
		//hdmi_ctrl->palette[color/2] |= green << 20 ;
		hdmi_ctrl->palette[color/2] |= green << 20 ;
		//hdmi_ctrl->palette[color/2] |= blue << 16;
		hdmi_ctrl->palette[color/2] |= blue << 16;
	}
